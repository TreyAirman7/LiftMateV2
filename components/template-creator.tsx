...
const SortableExerciseItem = ({ exercise, onRemove }: ExerciseItemProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-black rounded-lg mb-4 overflow-hidden border border-white">
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0 border-0 overflow-hidden bg-[#222222]">
        <div className="flex items-center justify-between p-4 border-b border-[#333333]">
          <h2 className="text-xl font-bold text-white">Add Exercise to Template</h2>
          <Button
            variant="ghost"
            size="icon"
...
const SetConfigurator = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0 border-0 overflow-hidden bg-[#222222]">
        <div className="flex items-center justify-between p-4 border-b border-[#333333]">
          <h2 className="text-xl font-bold text-white">{exercise?.name || "Configure Exercise"}</h2>
          <Button
            variant="ghost"
            size="icon"
            className="text-white hover:bg-white/10"
...
export default function TemplateCreator({
  ...
  useEffect(() => {
    if (templateToEdit && open) {
      ...
      setExercises(templateToEdit.exercises)
      setIsEditMode(true)
    } else if (open) {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="template-creator-dialog max-w-md p-0 border-0 overflow-hidden bg-[#222222]">
          <div className="flex items-center justify-between p-4 !bg-black">
            <h2 className="text-xl font-bold text-white">{isEditMode ? "Edit Template" : "Create New Template"}</h2>
            <Button
              variant="ghost"
              size="icon"
              className="text-white hover:bg-white/10"
